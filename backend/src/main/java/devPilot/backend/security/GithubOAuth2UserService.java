...
public class GithubOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String scopes = userRequest.getAccessToken().getScopes() != null
         ...
         User user = userService.upsertFromGitHub(githubUser.getAttributes() , accessToken , scopes);   // upsertFromGithub means we are taking some attributes, access token and scope and then inserting it into out DB
